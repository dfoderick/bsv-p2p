...
class Pool extends EventEmitter {
  ...
  constructor ( options ) {
    ...
    this.dnsSeeds = options.dnsSeeds ?? false;
  ...
  disconnect () {
    this.keepalive = false;
    for ( let connectedPeer of this._connectedPeers ) {
  ...
  _connectPeer ( addr ) {
    ...
    if ( !this._connectedPeers[ addr.hash ] ) {
      const port = addr.port || self.network.port;
  ...
  _addPeerEventHandlers ( peer, addr ) {
    ...
    peer.on( 'ready', function peerReady () {
      if ( this.dnsSeeds && !versions.includes( peer.version ) ) {
        peer.disconnect();
        delete self._connectedPeers[ addr.hash ];
  ...
  sendMessage ( message ) {
    // broadcast to peers
    for ( let key in this._connectedPeers ) {
