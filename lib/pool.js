 ...
 * @param {Array=} options.validVersions - The valid subversion of peers
 * @returns {Pool}
 * @constructor
 */
...
class Pool extends EventEmitter {
  ...
  constructor ( options ) {
    ...
    this.dnsSeeds = options.dnsSeeds ? options.dnsSeeds : this.network.dnsSeeds;
    this.relay = options.relay === false ? false : true;
    this.validVersions = options.validVersions ? options.validVersions : require( './versions' ).subversions;
  ...
  disconnect () {
    this.keepalive = false;
    for ( const key of Object.keys( this._connectedPeers ) ) {
  ...
  _addPeerEventHandlers ( peer, addr ) {
    peer.on('readmessage', (message) => {
      this.emit( 'readmessage', peer, message );
    })
    peer.on( 'disconnect', () => {
      this.emit( 'peerdisconnect', peer, addr );
    } );
    peer.on( 'ready', () => {
      if ( this.validVersions.includes( peer.subversion ) ) {
